...
UDP 소켓 전용 입출력 함수
- 네트워크 프로그래밍 실습 (UDP, Multicast, Broadcast)
     C 언어를 이용해 커맨드라인 인자 처리부터 UDP 소켓을 활용한 1:1, 1:N(멀티캐스트), 1:All(브로드캐스트) 통신 방식을 학습

1. 커맨드라인 인자 처리 (main.c)
    - argc(인자 개수)와 argv(인자 값 배열)를 활용하여 프로그램 실행 시 데이터를 전달하는 방법을 익혔습니다.
    - 네트워크 프로그램에서 포트 번호나 IP 주소를 동적으로 입력받을 때 필수적인 기초입니다.

2. UDP 기초 통신 (sendto.c, recvfrom.c)
    - 비연결형 서비스: SOCK_DGRAM 방식을 사용하여 연결 설정 없이 데이터를 전송합니다.
    - 주요 함수
        - sendto(): 대상의 주소 정보를 포함하여 데이터를 전송합니다
        - recvfrom(): 데이터를 수신함과 동시에 상대방의 주소 정보도 함께 저장합니다

3. 멀티캐스트 통신 (multicast_sender.c, multicast_receiver.c)
    - 개념: 특정 그룹(Multicast IP: 239.0.0.1 등)에 가입한 사용자들에게만 데이터를 전송합니다.
    - 핵심 설정
        - struct ip_mreq: 멀티캐스트 그룹 정보를 담는 구조체입니다
        - IP_ADD_MEMBERSHIP: 소켓 옵션을 통해 특정 멀티캐스트 그룹에 가입(Join)합니다.
        - SO_REUSEADDR: 동일한 포트에서 여러 수신 프로세스가 동작할 수 있도록 설정합니다.

4. 브로드캐스트 통신 (broadcast_sender.c, broadcast_receiver.c)
    - 개념: 동일 네트워크 내의 모든 호스트에게 데이터를 전송합니다.
    - 핵심 설정:
        - SO_BROADCAST: 기본적으로 금지된 브로드캐스트 전송을 허용하기 위해 소켓 옵션을 활성화해야 합니다.
        - 전송 주소: 255.255.255.255를 사용하여 전체 네트워크를 대상으로 설정합니다


### 1. 네트워크 프로토콜의 기초

- 프로토콜(Protocol): 서로 다른 시스템이 소통하기 위한 통신 규약(약속).

- TCP/IP 4계층
    1. 응용 계층 (Application): HTTP(웹), FTP(파일), SMTP(메일) 등 서비스 제공
    2. 전송 계층 (Transport): TCP(신뢰성), UDP(속도) 등 데이터 전송 제어
    3. 인터넷 계층 (Internet): IP 주소 지정 및 최적의 경로 설정(라우팅).
    4. 네트워크 액세스 계층 (Network Access): 물리적인 하드웨어 연결 및 데이터 전송

2. 소켓(Socket)의 전송 특성

- 스트림 소켓 (TCP): 1:1 연결형, 높은 신뢰성, 순서 보장, 데이터 경계 없음(바이트 스트림)
- 데이터그램 소켓 (UDP): 비연결형, 빠른 속도, 순서 무관, 데이터 경계 있음(메시지 단위)

3. TCP 서버 구현 함수 호출 순서 (중요!)
    1. socket(): 소켓 생성 (전화기 개설)
    2. bind(): 주소 및 포트 할당 (번호 부여)
    3. listen(): 연결 요청 대기 (통신 대기 상태)
    4. accept(): 연결 허용 (전화 받기, 통신용 소켓 생성)
    5. read() / write(): 데이터 송수신 (대화하기)
    6. close(): 소켓 닫기 (전화 끊기)

4. 실습: HTTP 웹 서버 구현 핵심 로직

직접 구현하신 웹 서버의 핵심 포인트는 "HTTP 요청 분석 및 분기 처리"였습니다.

- GET 요청 처리
    - 브라우저가 주소를 입력해 접속하면 서버는 준비된 HTML 문자열(webpage)을 전송함.
    - 이때 헤더에 Content-Type: text/html을 명시해야 브라우저가 웹 페이지로 인식함

- 이미지 전송 (이진 파일 처리):
    - 브라우저가 HTML 내의 <img> 태그를 읽고 다시 GET /good.jpg를 요청함.
    - 서버는 파일을 바이너리 모드(rb)로 열어 데이터를 바이트 단위로 읽어 전송함.
    - 헤더에 Content-Type: image/jpeg를 명시해야 이미지가 화면에 출력됨.

- POST 요청 처리:
    - 데이터 전송 요청(POST)이 들어오면 특정 메시지(예: "POST Success")를 응답하여 GET과 다르게 동작함을 증명함.

```
[실습 결과 보고]
1. 서버 환경: C 언어 기반의 리눅스(Ubuntu) TCP 소켓 서버 구현.
2. HTTP 처리: strncmp와 strstr 함수를 사용하여 클라이언트의 요청 라인(Request Line)을 분석함.
3. GET 방식: 메인 페이지 요청 시 HTML 데이터를 전송하고, 이미지 요청 시 fopen을 통해 good.jpg 바이너리 데이터를 읽어 전송함.
4. POST 방식: 데이터 전송 요청 시 별도의 응답 메시지를 전송하여 GET과 분리된 동작을 확인함.
5. 결과: 웹 브라우저를 통해 노란색 배경의 HTML 페이지와 이미지가 정상 출력됨을 확인하였음.
```
