...
- 멀티 스레드 - 동기화 : 여러개의 스레드가 하나의 (파일,변수)를 동시에 쓸려고 할려고 할때 순서를 정해주거나 한번에 하나씩만 접근하게 하는것
- 멀티플레싱
- UDP

- 스레드 : 메모리 공유
- 동기화: 순서 맞추는거
- 뮤텍스 ; 열쇠를 가진 딱 한 명(스레드)만 들어가고, 나머지는 줄 서서 기다림
- 세마포어 : 허용된 개수만큼 들어갈 수 있고, 신호를 주고받으며 작업을 이어감

1. 동기화의 기초 (thread03.c, thread04.c)

- 뮤텍스 활용: thread03.c에서 sum 변수를 보호하기 위해 lock과 unlock을 적재적소에 사용하셨습니다
- thread04.c에서 일부러 뮤텍스를 주석 처리해 나중에 값이 0이 나오지 않는 것을 확인하면서 뮤텍스의 중요성을 체감하셨을 거예요.

2. 스레드 간 신호 주고받기 (sema.c)
- 세마포어의 역할: 입력을 받는 스레드와 계산하는 스레드가 번갈아 가며 동작하도록 sem_one, sem_two 두 개의 신호등을 아주 잘 설계하셨습니다.

3. 멀티 스레드 채팅 응용 (chat_sv.c, chat_clnt.c)

- 서버 측 (chat_sv.c): 다수의 클라이언트를 배열(clnt_socks)로 관리하며, 새로운 연결이 들어오거나 끊길 때 뮤텍스로 배열을 보호하여 안정적인 1:N 채팅 서버를 구축했습니다.

- 클라이언트 측 (chat_clnt.c): 송신(send)과 수신(recv)을 각각 독립된 스레드로 분리하여, 메시지를 보내는 중에도 실시간으로 상대방의 메시지를 받을 수 있게 구현했습니다.

4. I/O 효율화 기술 (select.c, epoll_server.c)

- 멀티플렉싱 기초 (select.c): select 함수를 이용해 5초 동안 입력이 없으면 Time-out이 발생하는 구조를 실습하며, 한 명의 일꾼이 여러 입력을 감시하는 원리를 이해했습니다.

- 고성능 서버 (epoll_server.c): select의 성능 한계를 극복한 리눅스 전용 epoll 방식을 실습하며, 수많은 클라이언트 접속을 효율적으로 처리하는 현대적인 서버 구조를 익혔습니다.
